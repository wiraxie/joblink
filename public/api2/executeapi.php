...
{	
	...
	$getid = "SELECT id FROM personal_uploads ORDER BY id DESC LIMIT 1;";
...
//transkrip nilai
function uploadtranskripnilai_post_form($dt)
{	
	$target_dir = "../../uploads/";
	$target_file = $target_dir . basename($_FILES["photo"]["name"]);
	$uploadOk = 1;
	...
	else
	{
		...
		{	
			if(move_uploaded_file($_FILES["photo"]["tmp_name"], $newfile)) 
			{
				$query = "INSERT INTO personal_uploads(regid, title, photourl, created_at) VALUES(".$regid.", 'Transkrip Nilai', 'uploads/".$new_target_file."', now())";
				run_sql($query);
...
function updateitranskripnilai_post_form($dt)
{	
	$target_dir = "../../uploads/";
	$target_file = $target_dir . basename($_FILES["photo"]["name"]);
	$uploadOk = 1;
